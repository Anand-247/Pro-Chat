...
import { updateChatLatestMessage, deleteChatAction, clearChatAction, removeChatLocally, setActiveChat, setTypingChat } from "../store/chatSlice";
import { Send, Smile, Paperclip, MoreVertical, Loader2, Trash2, Clock, Check, CheckCheck, Edit2, Trash, X, Info, LogOut, UserMinus, Plus } from "lucide-react";
...
const ChatWindow = () => {
  ...
  const currentUserMemberObj = activeChat?.users?.find(u => u.id === user.id)?.ChatMember;
  const hasLeftChat = activeChat?.isGroupChat && currentUserMemberObj && currentUserMemberObj.deletedAt !== null;

  // Filter out users who have left the group for rendering active participant details
  const activeMembers = activeChat?.users?.filter(u => !u.ChatMember?.deletedAt) || [];
    ...
    <div className="glass-container h-full w-full flex flex-col overflow-hidden relative">
              ...
              <div className="p-6 flex flex-col items-center">
                 ...
                 {isRenaming ? (
                    ...
                    <h3 className="text-2xl font-bold text-white mb-1 flex items-center gap-2">
                       {getChatName()}
                       {activeChat.isGroupChat && activeChat.groupAdminId === user.id && (
                 ...
                 {activeChat.isGroupChat && (
                    <div className="w-full mt-6">
                       ...
                       {activeChat.groupAdminId === user.id && !hasLeftChat && (
                             ...
                             <div key={u.id} className="flex items-center gap-3 p-2 hover:bg-white/5 rounded-lg transition-colors group/member">
                                ...
                                <div className="flex-1">
                                   ...
                                   <p className="text-[10px] text-slate-500">
                                      {u.id === activeChat.groupAdminId ? "Group Admin" : "Member"}
                                      {u.ChatMember?.createdAt && ` • Joined ${new Date(u.ChatMember.createdAt).toLocaleDateString()}`}
                                   </p>
                                </div>
                                {activeChat.groupAdminId === user.id && u.id !== user.id && (
                                   <button 
                                     onClick={() => handleRemoveUser(u.id)} 
                                     className="opacity-0 group-hover/member:opacity-100 p-2 text-red-500 hover:bg-red-500/20 rounded-full transition-all"
                                   >
                                      <UserMinus size={16} />
                                   </button>
                                )}
      ...
      {!hasLeftChat ? (
         ...
         <div className="p-4 border-t border-white/10 bg-slate-900/60 z-10 text-center">
            <p className="text-slate-400 text-sm italic">You are no longer a participant in this group.</p>
         </div>
      )}
    </div>
  );
